...
const MovieSectionCard = ({ section, onClick, userPlan, moviesInSection = [] }) => {
  ...
  useEffect(() => {
    ...
    if (moviesInSection && moviesInSection.length > 0) {
      ...
      const initialMovieWithThumb = moviesInSection.find(m => m.thumbnail || m.logo);
      if (initialMovieWithThumb) {
        setCurrentThumbnail(initialMovieWithThumb.thumbnail || initialMovieWithThumb.logo);
      ...
      if (moviesInSection.length > 1) { // Solo rotar si hay más de una película con thumbnail
        intervalId = setInterval(() => {
          const moviesWithThumbnails = moviesInSection.filter(m => m.thumbnail || m.logo);
          if (moviesWithThumbnails.length > 0) {
            const randomIndex = Math.floor(Math.random() * moviesWithThumbnails.length);
            const newMovieThumbnail = moviesWithThumbnails[randomIndex]?.thumbnail || moviesWithThumbnails[randomIndex]?.logo;
