...
// --- Componentes UI (asumo que los tienes definidos como antes) ---
...
// Planes (debe coincidir con el enum del modelo Channel.js y User.js)
const ALL_AVAILABLE_PLANS = [
  ...
  { key: "free_preview", displayName: "Vista Previa Gratuita" },
];
// Planes para VOD (si es diferente y un solo plan)
...
export default function AdminPanel() {
  ...
  // Estados VOD (sin cambios mayores aquí, a menos que VOD también necesite multiplan/section)
  ...
  const [videoRequiresPlan, setVideoRequiresPlan] = useState(VOD_PLAN_OPTIONS[0]?.key || "basico");
  ...
  const [channelSection, setChannelSection] = useState("General"); // CAMBIO: campo de texto para la sección
  ...
  const [channelIsPubliclyVisible, setChannelIsPubliclyVisible] = useState(true);
  ...
  const fetchAdminChannelsList = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Fallo al cargar canales de admin."); setChannels([]);
    } finally { setIsLoadingList(false); }
  }, [token]);

  const clearChannelForm = useCallback(() => {
    setChannelId(null); setChannelName(""); setChannelUrl(""); setChannelLogo("");
    setChannelDescription(""); setChannelSection("General"); // Default para la sección
    setChannelIsActive(true); setChannelIsFeatured(false);
    setChannelRequiresPlan([]);
    setChannelIsPubliclyVisible(true);
    clearMessages();
  }, []);
  ...
  const handleEditChannelClick = useCallback((channel) => {
    if (!channel || (!channel.id && !channel._id)) { setErrorMsg("ID de canal inválido."); return; }
    setChannelId(channel.id || channel._id);
    setChannelName(channel.name || "");
    setChannelUrl(channel.url || "");
    setChannelLogo(channel.logo || "");
    setChannelDescription(channel.description || "");
    setChannelSection(channel.section || "General"); // Usar channel.section
    ...
    setChannelIsPubliclyVisible(channel.isPubliclyVisible === undefined ? true : channel.isPubliclyVisible);
  ...
  const submitChannel = async (e) => {
    e.preventDefault();
    if (!channelName || !channelUrl || !channelSection.trim()) {
      setErrorMsg("Nombre, URL y Sección del canal son requeridos.");
      return;
    }
    setIsSubmitting(true); clearMessages();
    const channelData = {
      name: channelName, url: channelUrl, logo: channelLogo, description: channelDescription,
      section: channelSection.trim(), // Enviar 'section'
      active: channelIsActive, isFeatured: channelIsFeatured,
      requiresPlan: channelRequiresPlan.length > 0 ? channelRequiresPlan : ['gplay'],
    ...
    try {
      ...
      clearChannelForm(); fetchAdminChannelsList(); setActiveTab("manage_channels");
  ...
  const handleDeleteChannel = async (id) => {
    ...
    try {
      ...
      setSuccessMsg("Canal eliminado."); fetchAdminChannelsList();
  ...
  // --- Funciones para VOD (Mantener como estaban o adaptar según necesidad) ---
  const fetchVideosList = useCallback(async () => { /* ... tu código ... */ }, [token]);
  const clearVodForm = useCallback(() => { /* ... tu código ... */ }, []);
  const handleEditVodClick = useCallback((video) => { /* ... tu código ... */ }, []);
  const submitVideo = async (e) => { /* ... tu código ... */ };
  const deleteVideoHandler = async (id) => { /* ... tu código ... */ };

  useEffect(() => {
    clearMessages();
    if (user && user.role === 'admin' && token) {
      if (activeTab === "manage_channels") { fetchAdminChannelsList(); if (!channelId) clearChannelForm(); }
      else if (activeTab === "add_channel" && !channelId) { clearChannelForm(); }
      else if (activeTab === "manage_vod") { fetchVideosList(); if (!vodId) clearVodForm(); }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-8 max-w-5xl mx-auto bg-gray-900 text-white min-h-screen">
      ...
      {errorMsg && <div className="p-3 my-2 bg-red-800 text-red-100 rounded shadow" role="alert">{errorMsg}</div>}
      {successMsg && <div className="p-3 my-2 bg-green-800 text-green-100 rounded shadow" role="alert">{successMsg}</div>}
      ...
      {/* Pestaña Subir M3U (Sin cambios visuales mayores aquí) */}
      ...
      {activeTab === "add_channel" && (
        <section className="p-6 bg-gray-800 rounded-lg shadow-xl">
          ...
          <form onSubmit={submitChannel} className="space-y-6">
            ...
            {/* CAMBIO: Input de texto para la Sección */}
            <Input type="text" placeholder="Sección (Ej: Deportes, Noticias, Pelis HD)" value={channelSection} onChange={(e) => setChannelSection(e.target.value)} required />
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Planes Requeridos</label>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {ALL_AVAILABLE_PLANS.map(plan => (
                  <label key={plan.key} className="flex items-center space-x-2 text-gray-300 cursor-pointer p-2 bg-gray-700 rounded-md hover:bg-gray-600">
                    <input type="checkbox" value={plan.key} checked={channelRequiresPlan.includes(plan.key)} onChange={() => handleChannelPlanChange(plan.key)}
                      className="form-checkbox h-4 w-4 text-red-500 bg-gray-600 border-gray-500 rounded focus:ring-red-500" />
            ...
            <div className="space-y-3 pt-2">
                <label className="flex items-center space-x-2 text-gray-300 cursor-pointer">
                    <input type="checkbox" checked={channelIsPubliclyVisible} onChange={(e) => setChannelIsPubliclyVisible(e.target.checked)} className="form-checkbox h-5 w-5 text-red-500"/>
                    <span>Visible Públicamente (Todos ven la tarjeta, acceso según plan)</span>
                </label>
                <label className="flex items-center space-x-2 text-gray-300 cursor-pointer">
                    <input type="checkbox" checked={channelIsActive} onChange={(e) => setChannelIsActive(e.target.checked)} className="form-checkbox h-5 w-5 text-red-500"/>
                    <span>Activo</span>
                </label>
                <label className="flex items-center space-x-2 text-gray-300 cursor-pointer">
                    <input type="checkbox" checked={channelIsFeatured} onChange={(e) => setChannelIsFeatured(e.target.checked)} className="form-checkbox h-5 w-5 text-red-500"/>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-3">
              <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto bg-green-600 hover:bg-green-700">
                {isSubmitting ? (channelId ? "Actualizando..." : "Agregando...") : (channelId ? "Guardar Cambios" : "Agregar Canal")}
              </Button>
              ...
              {!channelId && <Button type="button" onClick={clearChannelForm} className="w-full sm:w-auto bg-yellow-600 hover:bg-yellow-700 text-black">Limpiar</Button>}
            </div>
          </form>
        </section>
      )}
      ...
      {activeTab === "manage_channels" && (
        <section className="p-6 bg-gray-800 rounded-lg shadow-xl">
          <h2 className="text-2xl font-semibold mb-4">Gestionar Canales Existentes</h2>
            {isLoadingList ? (<p className="text-center py-10">Cargando canales...</p>)
            : (channels && channels.length > 0) ? (
            <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar"> {/* custom-scrollbar si tienes estilos para ello */}
              {channels.map((ch) => (
                <div key={ch.id || ch._id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 bg-gray-700 hover:bg-gray-600/50 rounded-md gap-3">
                  ...
                  <div className="flex-grow mb-2 sm:mb-0 text-sm min-w-0">
                    ...
                    <p className="text-xs text-gray-500">Sección: {ch.section || "Sin Sección"}</p>
            ...
            ) : (<p className="text-center py-10">{errorMsg ? `Error: ${errorMsg}` : "No hay canales para mostrar."}</p>)}
        </section>
      )}

      {/* Pestañas VOD (Sin cambios mayores, a menos que necesites adaptar VOD también) */}
      ...
      {activeTab === "add_vod" && (
        <section className="p-6 bg-gray-800 rounded-lg shadow-xl">
          ...
          <form onSubmit={submitVideo} className="space-y-6">
            ...
            <div className="space-y-3 pt-2">
                <label className="flex items-center space-x-2 text-gray-300 cursor-pointer">
                    <input type="checkbox" checked={videoIsFeatured} onChange={(e) => setVideoIsFeatured(e.target.checked)} className="form-checkbox h-5 w-5 text-red-500"/>
                    <span>Destacado (Home)</span>
                </label>
                <label className="flex items-center space-x-2 text-gray-300 cursor-pointer">
                    <input type="checkbox" checked={videoIsActive} onChange={(e) => setVideoIsActive(e.target.checked)} className="form-checkbox h-5 w-5 text-red-500"/>
                    <span>Activo</span>
                </label>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 pt-3">
              <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto bg-green-600 hover:bg-green-700">
                {isSubmitting ? (vodId ? "Actualizando VOD..." : "Agregando VOD...") : (vodId ? "Guardar Cambios VOD" : "Agregar VOD")}
              </Button>
              ...
              {!vodId && <Button type="button" onClick={clearVodForm} className="w-full sm:w-auto bg-yellow-600 hover:bg-yellow-700 text-black">Limpiar</Button>}
            </div>
          </form>
        </section>
      )}
