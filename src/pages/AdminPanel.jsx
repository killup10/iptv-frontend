...
export default function AdminPanel() {
  ...
  const submitChannel = async e => {
    ...
    try {
      ...
      setSuccessMsg('Canal agregado'); setErrorMsg(''); setChannelName(''); setChannelUrl(''); fetchChannels();
  ...
  const submitVideo = async e => {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) throw new Error(data.error);
      setSuccessMsg('Video agregado'); setErrorMsg(''); setVideoTitle(''); setVideoUrl('');
    } catch (err) { setErrorMsg(err.message); }
    finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="p-6 space-y-8 max-w-4xl mx-auto bg-gray-900 text-white">
      <h1 className="text-3xl font-bold">Panel de Administraci&oacute;n</h1>
      ...
      {activeTab === 'm3u' && (
        <form onSubmit={submitM3u} className="p-6 bg-gray-800 rounded-lg">
          ...
          <input type="file" accept=".m3u,.txt" onChange={e=>setM3uFile(e.target.files[0])}
            className="block w-full text-sm text-white file:bg-red-600 file:text-white file:py-2 file:px-4 rounded" />
          <Button type="submit" disabled={isSubmitting} className="mt-4 bg-blue-600">{isSubmitting?'Subiendo...':'Subir M3U'}</Button>
        </form>
      )}
      ...
      {activeTab === 'channels' && (
        <form onSubmit={submitChannel} className="p-6 bg-gray-800 rounded-lg space-y-4">
          ...
          <Button disabled={isSubmitting} className="bg-blue-600">{isSubmitting?'Agregando...':'Agregar Canal'}</Button>
        </form>
      )}
      ...
      {activeTab === 'vod' && (
        <form onSubmit={submitVideo} className="p-6 bg-gray-800 rounded-lg space-y-4">
          ...
          <Input placeholder="T&iacute;tulo" value={videoTitle} onChange={e=>setVideoTitle(e.target.value)} className="bg-gray-700" />
          <Input placeholder="URL MP4/MKV" value={videoUrl} onChange={e=>setVideoUrl(e.target.value)} className="bg-gray-700" />
          <Button disabled={isSubmitting} className="bg-blue-600">{isSubmitting?'Procesando...':'Agregar VOD'}</Button>
        </form>
      )}
        ...
        <div className="p-6 bg-gray-800 rounded-lg">
          ...
          {channels.map(ch => (
            <div key={ch._id} className="flex justify-between mb-2">
              <span>{ch.name}</span>
              <Button onClick={()=>{/* eliminar */}} className="bg-red-600">Eliminar</Button>
            </div>
          ))}
        </div>
      )}
